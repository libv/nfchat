...
extern void handle_command (char *, struct session *, int, int);
extern int tcp_send_len (struct server *, char *, int);
extern int hook_signal (struct xp_signal *);
extern void unhook_signal (struct xp_signal *);
extern int for_files (char *, char *, void callback (char *));
extern char *get_xdir ();
...
#undef F
	...
	PyEval_CallObject (hook->cback, arglist);
	if (PyErr_Occurred())
		PyErr_Print();
...
#define F(x, y) PyDict_SetItemString (map, x, PyInt_FromLong(self->sess->y))
	...
	F("is_shell", is_shell);
	F("is_tab", is_tab);
	F("is_dialog", is_dialog);
	F("is_server", is_server);
#undef F
	
	PyDict_SetItemString (map, "servername", PyString_FromString(self->sess->server->hostname));
	PyDict_SetItemString (map, "serverport", PyInt_FromLong (self->sess->server->port));
	PyDict_SetItemString (map, "servernick", PyString_FromString (self->sess->server->nick));
   ...
   list = self->sess->userlist;
	while (list) {
      cur = (struct user *)list->data;
		tuple = PyTuple_New (3);
		g_assert (cur->hostname);
		PyTuple_SetItem (tuple, 0,
				 PyString_FromString (cur->hostname));
...
#ifdef USE_IMLIB
	PyDict_SetItemString (map, "Imlib", PyString_FromString("yes"));
#else
	PyDict_SetItemString (map, "Imlib", PyString_FromString("no"));
#endif
...
static PyMethodDef _xchat_methods[] = {
	...
	{"get_current_session", (PyCFunction) pysH_get_current_session, METH_VARARGS},
...
pys_autoloadfile (char *file)
{
   ...
   handle_command (buf, sess_list->data, FALSE, FALSE);
   free (buf);
}
...
pys_load (struct session *sess, char *tbuf, char **word, char **word_eol)
{
	...
	in = fopen (word[2], "r");
	if (!in) {
		PrintText (current_tab, "\017Python interpreter failed to load script\n");
		return FALSE;
	}
	if ( (tstate = Py_NewInterpreter ()) == NULL) {
		PrintText (current_tab, "\017Python interpreter failed to load\n");
		return TRUE;
	}
	Py_InitModule("XChat", _classes);
	if (PyErr_Occurred()) {
		PrintText (current_tab, "\017Python interpreter failed to import XChat\n");
		return TRUE;
	}
